...
- 커밋: `e58d544` + `4940837` (main 직접, 푸시됨). 0007 DB 적용 완료(사용자 psql)
...
- 검증: 테스트 127개(신규 4 — challenge 매핑/레거시 none/insert/부분 update) 그린. typecheck/lint 그린. 실기기(SM-S928N, 9/3 밤): 강제 기상 프리셋 → challenge=math DB 저장 ✓ / 발화 시 알림 액션 스누즈만(해제 없음) ✓ / 알림 탭 → 알람 화면 "밀어서 문제 풀기" → 수학 3문제 오버레이 → 정답 후 해제(FGS 0) ✓ / 카드 탭 편집 폼 프리필 ✓ / 삭제 → "되돌리기" 스낵바 ✓
- 발견한 함정: ① **1회성 순수 알람이 울린 뒤 다시 예약됨** — 재계산이 "다음 발화"만 보므로 사실상 매일 반복. DELIVERED에서 headless로 enabled=false 갱신(`src/data/oneshot.ts`, 4940837) ② Android 시간 피커 다이얼로그는 adb 키 입력에 닫힘 — 실기기 테스트는 DB에서 time 직접 갱신 후 포그라운드 재진입으로 우회 ③ opus 서브에이전트가 500으로 2회 중단 — 디스크 상태는 온전해서 메인이 마무리
- 미검증: 흔들기 챌린지(가속도), 24시간 표기·주 시작 일요일 캘린더 렌더, 브리핑 주말 제외, 검색/시간표에서 '이 일정만' 회차
