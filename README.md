...
- **검색 / 백업** — 제목·메모 검색, JSON 내보내기·가져오기 + `.ics` 내보내기·**가져오기**, 주 1회 자동 로컬 백업
- **빠른 추가** — "내일 오후 3시 팀플" 자연어 입력 → 일정 생성. 규칙 기반 파서(정규식+date-fns), AI API 미사용
- **아침 브리핑** — 기상 시각에 오늘 일정 요약 (저녁 브리핑과 별도 토글)
- **점진적 볼륨** — 알람이 30초에 걸쳐 서서히 커지는 옵션
- **시험기간 모드** — 홈 마감 임박 창을 3일 → 7일로 확장
- **웹 앱** — 주간/월간 뷰, 드래그로 생성·이동·리사이즈. 도메인 로직을 앱과 100% 공유.
  모바일 브라우저에서도 터치로 드래그(포인터 이벤트)
- **약속 잡기(when2meet)** — 후보 날짜×시간 그리드를 칠해 겹치는 시간을 찾고 확정하면 일정 생성.
  참여자는 **계정 없이** 링크만으로 참여
- **알림에서 바로 제어** — 알람 알림의 끄기/스누즈 액션, 조용 모드(지정 시각까지 알람 일시 중지),
  한 번만 건너뛰기, 알림별 소리·켬끔 개별 설정
- **백업 안전망** — 자동 백업을 사용자가 고른 폴더에도 복사(앱 삭제 생존), 복원은 서버 트랜잭션 한 번
- 다크/라이트 테마, 햅틱, 스켈레톤 로딩

> 운영 비용 0원 원칙: Supabase 무료 티어 + 로컬 알람 엔진 + (선택) Vercel Hobby 웹 배포.
> 유료 API·AI 토큰을 쓰는 기능은 없다.
...
pnpm verify           # test + typecheck + lint + 웹 빌드 (CI와 같은 순서)
```

GitHub Actions(`.github/workflows/ci.yml`)가 push·PR마다 `pnpm verify`와 같은 검사를 돌린다.
